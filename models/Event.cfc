...
component extends="Model" hint="Main Event Object"
{
	...
	*/
	public void function checkDates() {
    	...
    	if(!isDate(this.endDate)){
    		...
    		this.endDate=dateAdd("h", 1, this.start);
    	}
    	// Don't allow end date to be before start date
    	// We check for -1 to allow for end dates on the same day
    	if((DateCompare("#this.endDate#", "#this.start#")) EQ -1 ){
			 addError(property="endDate", message="End Date can not be before Start Date.");
		}
	}
	...
	*/
	public void function formatDates() {
		...
		if(structKeyExists(this, "endDate")){
			this.endDate=dateFormat(this.endDate, "DD MMM YYYY") & ' ' & timeFormat(this.endDate, "HH:mm");
		}
	}
