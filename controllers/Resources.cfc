...
/******************** Ajax/Remote/Misc*************/
	...
	*/
	public void function checkavailability() {
		...
		param name="params.endDate" default="" type="string";
		if(!isDate(params.endDate)){
			params.endDate=dateAdd("h", 1, params.start);
		}
		...
		checkEvent=model("event").findAll(
			where="start <= '#params.start#' AND end >= '#params.endDate#' AND id != #params.eventid# AND resourceid = #params.id#",
		...
		/* Check for events with an All Day flag which might have incorrect timings set; this shouldn't affect events which span multiple days, as the above check (should) pick them up;*/
			...
			tempend=dateFormat(params.endDate, "yyyy-mm-dd") & ' ' & timeFormat(params.endDate, "23:59");
