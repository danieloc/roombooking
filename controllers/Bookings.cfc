	...
	public void function day() {
		...
		for(location in locations){
			...
			do {
			    ...
			    eventsQ.addParam(name='endDate', value=t.timeslot, cfsqltype='cf_sql_timestamp');
			    eventsQ.setSQL('SELECT * FROM rs WHERE locationid =:locationid AND start <= :start AND [endDate] > :endDate AND allday = 0');
			    locationEvents = eventsQ.execute().getResult();
 				if(locationEvents.recordcount){
					 if(tempid NEQ locationEvents.id){
					 	...
					 	if(day(locationEvents.start) != day(locationEvents.endDate)){
					 		t.isMultiday=true;
					 	} else {
					 		t.isMultiday=false;
					 	}
					 	// Check for multiday event with specific end time
					 	if(t.isMultiday	AND	(day(day.thedate) EQ day(locationEvents.endDate))){
					 		t.duration=DateDiff("n", t.timeslot, locationEvents.endDate);
							...
							t.duration=DateDiff("n",day.thedate, locationEvents.start);
					 		...
					 		t.duration=DateDiff("n", locationEvents.start, locationEvents.endDate);
					 	}
						...
						if(t.isMultiday){
							...
							t.content=t.content & "#timeFormat(locationEvents.start, "HH:MM")# - #timeFormat(locationEvents.endDate, "HH:MM")# "
					...
					AND params.repeatno GTE 1)
				{
					for (i = 1; i lte params.repeatno; i = i + 1){
						...
						if(params.repeat EQ "week"){
							...
							if(isDate(nevent.endDate)){
								nevent.endDate = dateAdd("d", (i * 7), nevent.endDate);
							}
						...
						if(params.repeat EQ "month"){
							...
							if(isDate(nevent.endDate)){
						  		nevent.endDate = dateAdd("m", i, nevent.endDate);
						  	}
						}
	...
	*/
	public void function check() {
		if(structKeyExists(params, "start")
			AND structKeyExists(params, "endDate")
			...
			if(len(params.id)){
				eCheck=model("event").findAll(where="status != 'denied' AND id != #params.id# AND start <= '#params.start#' AND endDate >= '#params.start#' AND locationid = #params.location#");
			} else {
				eCheck=model("event").findAll(where="status != 'denied' AND start <= '#params.start#' AND endDate >= '#params.start#' AND locationid = #params.location#");
			}
